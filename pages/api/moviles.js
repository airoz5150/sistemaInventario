...
export default async function handler(req, res) {
  ...
  switch (method) {
    ...
    case 'POST':
      try {
        const moviles = new Moviles(req.body);
        await moviles.save();
        res.status(201).json({ success: true, data: moviles });
      } catch (error) {
        res.status(400).json({ success: false, error: error.message });
      }
      break;
      ...
      try {
        const { id } = req.query;
        const deletedMoviles = await Moviles.findByIdAndDelete(id);
      ...
      case "PUT":
        try {
          ...
          const result = await Moviles.findByIdAndUpdate(id, updatedData, { new: true });
