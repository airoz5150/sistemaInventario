...
import Bocina from '../../models/bocina';
  ...
  switch (method) {
    case 'GET':
      try {
        ...
        const bocina = await Bocina.find({});
        res.status(200).json({ success: true, data: bocina });
      } catch (error) {
    ...
    case 'POST':
      try {
        const bocina = new Bocina(req.body);
        await bocina.save();
        res.status(201).json({ success: true, data: bocina });
      } catch (error) {
        res.status(400).json({ success: false, error: error.message });
      }
      break;
      ...
      try {
        const { id } = req.query;
        const deletedBocina = await Bocina.findByIdAndDelete(id);
      ...
      case "PUT":
        try {
          ...
          const result = await Bocina.findByIdAndUpdate(id, updatedData, { new: true });
