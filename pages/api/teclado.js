  ...
  switch (method) {
    case 'GET':
      try {
        ...
        const teclado = await Teclado.find({});
        res.status(200).json({ success: true, data: teclado });
      } catch (error) {
    ...
    case 'POST':
      try {
        ...
        await teclado.save();
        res.status(201).json({ success: true, data: teclado });
      } catch (error) {
        res.status(400).json({ success: false, error: error.message });
      }
      break;
      ...
      try {
        const { id } = req.query;
        const deletedTeclado = await Teclado.findByIdAndDelete(id);
      ...
      case "PUT":
        try {
          ...
          const result = await Teclado.findByIdAndUpdate(id, updatedData, { new: true });
